...
function showSearchHistory(){
  ...
  var h='<div class="sh-header"><span>搜索历史</span><button id="shClearBtn">清空</button></div>';
  for(var i=0;i<Math.min(list.length,10);i++){
    h+='<div class="sh-item" data-kw="'+escapeHtml(list[i]).replace(/"/g,'&quot;')+'">'+escapeHtml(list[i])+'</div>';
  }
  box.innerHTML=h;
  box.style.display='block';

  // Event delegation
  document.getElementById('shClearBtn').addEventListener('click', function(e) {
    e.stopPropagation();
    if (window.clearSearchHistory) window.clearSearchHistory();
  });
  box.querySelectorAll('.sh-item').forEach(function(item) {
    item.addEventListener('click', function() {
      var kw = this.getAttribute('data-kw');
      if (window.selectSearchHistoryItem) window.selectSearchHistoryItem(kw);
    });
  });
}
...
// === Main Page Switch ===
// 注意: nc-movie-engine.js 中也有同名函数，该文件加载较早，会被此版本覆盖。
// 此处合并了两者的完整逻辑。
function switchMainPage(page){
  document.querySelectorAll('.main-page').forEach(function(el){el.classList.toggle('active',el.getAttribute('data-main')===page || (page==='detail' && el.getAttribute('data-main')==='home'))});
