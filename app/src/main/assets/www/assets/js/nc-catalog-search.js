...
(function(){
  ...
  // 仅在 app.js 未覆盖时注册这些函数（向后兼容）
  if(!window._ncAppRenderMovieHome){
  ...
  // 保留工具函数，不覆盖 app.js 的全局渲染函数
  // renderTvCats / renderMovieHome / tvSetCat / tvShowAll / tvSearch 由 app.js 统一管理

  var oldRenderMovieHome=window.renderMovieHome;
  if(!oldRenderMovieHome||!window._ncAppRenderMovieHome){
    window.renderMovieHome=function(){
      if(oldRenderMovieHome)oldRenderMovieHome.apply(this,arguments);
    };
  }
  ...
  } // end if(!window._ncAppRenderMovieHome)
