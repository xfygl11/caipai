...
function makeGroups(k){
  var gs=[],seen={};
  for(var i=0;i<5;i++){
    var p=predict(k,i),key=p.f.join(',')+'+'+p.b.join(',');
    var guard=0;
    while(seen[key]&&guard<8){p=predict(k,i);key=p.f.join(',')+'+'+p.b.join(',');guard++}
    seen[key]=1;gs.push(p);
  }
  return gs;
}
...
function renderTab(k,c,hist,ph){
  ...
  h.push('<div class="fstat">模式: <span id="'+k+'-fs">'+(isAppMode()?'APP模式':(online?'联网模式':'本地模式'))+'</span><button class="sync-btn" onclick="fetchAPI(\''+k+'\')">同步开奖</button></div>');
  h.push('<div class="cpred"><div class="cpred-t">本期五组预测 <button class="copy-btn" onclick="copyPred(\''+k+'\',\''+entry.p+'\',true)">复制五组</button></div>');
  entry.groups.forEach(function(g,idx){
    ...
    h.push('<button class="copy-btn" onclick="copyPred(\''+k+'\',\''+entry.p+'\',false,'+idx+')">复制</button>');
  ...
  pred5.forEach(function(pr){
    ...
    h.push('<td><button class="expand-btn" onclick="togglePredDetail(\''+k+'\',\''+pr.p+'\')">展开</button></td></tr>');
    h.push('<tr class="pred-detail" id="'+k+'-detail-'+pr.p+'"><td colspan="8"><div class="pred-detail-box">');
    h.push('<div style="text-align:right;margin-bottom:6px"><button class="copy-btn" onclick="copyPred(\''+k+'\',\''+pr.p+'\',true)">复制本期五组</button></div>');
  ...
  hist.slice(0,10).forEach(function(r){
    ...
    h.push('</td><td>'+drawSalesText(r)+'</td><td>'+S(r.f)+'</td><td>'+odds+'/'+(r.f.length-odds)+'</td><td><button class="expand-btn" onclick="togglePredDetail(\''+k+'\',\'draw-'+r.p+'\')">查看详情</button></td></tr>');
  ...
  h.push('<button class="btn btn-o" onclick="genRandomNums(\''+k+'\')">随机生成</button>');
  h.push('<button class="copy-btn" onclick="copyRandomNums(\''+k+'\')">复制</button>');
  h.push('<button class="copy-btn" onclick="clearRandomNums(\''+k+'\')">清空</button></div></div>');
  ...
  h.push('<input type="text" id="'+k+'-paste" placeholder="粘贴整组号码，如 01 02 03 04 05 + 06 07" style="min-width:220px"><button class="btn btn-o" onclick="fillMainPaste(\''+k+'\')">填入</button>');
  ...
  h.push('<button class="btn btn-o" onclick="submitR(\''+k+'\')">确认</button></div></div>');

  // full history
  h.push('<div class="st" style="cursor:pointer" onclick="toggleH(\''+k+'\')">完整历史数据 ▾</div><div id="'+k+'-hist" style="display:none"></div>');
...
function renderAcc(k,c,ph){
  ...
  hist.forEach(function(h){
    ...
    h2.push('<td><span class="hb '+bc+'">'+badge+'</span><div style="margin-top:4px;color:'+(h.prize?'#4ade80':'#8899aa')+'">'+ptxt+'</div></td><td><button class="expand-btn" onclick="togglePredDetail(\''+k+'\',\'win-'+h.p+'\')">展开</button></td></tr>');
...
function ensurePred(k,p){
  ...
  entry={p:p,groups:makeGroups(k),checked:false,fh:0,bh:0};
  normalizeEntry(k,entry);
  ph.pending.push(entry);
...
function toggleH(k){
  ...
  if(el.style.display==='none'||el.style.display===''){
    ...
    hist.forEach(function(r){
      ...
      h.push(' ');
      r.b.forEach(function(n){h.push('<span class="nball b">'+P(n)+'</span>')});
      h.push('</td><td>'+drawSalesText(r)+'</td><td>'+S(r.f)+'</td><td><button class="expand-btn" onclick="togglePredDetail(\''+k+'\',\'full-'+r.p+'\')">查看详情</button></td></tr>');
...
function buildLotteryPage(id){
  ...
  var online=isOnlineMode();
  h.push('<div class="fstat">模式: <span id="'+id+'-fs">'+(isAppMode()?'APP模式':(online?'联网模式':'本地模式'))+'</span><button class="sync-btn" onclick="fetchLotteryAPI(\''+id+'\')">同步开奖</button></div>');
  h.push('<div class="cpred"><div class="cpred-t">本期五组预测 <button class="copy-btn" onclick="copyLotteryPred(\''+id+'\',\''+entry.p+'\',true)">复制五组</button></div>');
  entry.groups.forEach(function(g,idx){
    h.push('<div class="pred-line"><span class="pred-name">算法'+(idx+1)+'</span>'+lotPickHtml(g.pick,false)+'<button class="copy-btn" onclick="copyLotteryPred(\''+id+'\',\''+entry.p+'\',false,'+idx+')">复制</button><div class="cpred-m">'+g.name+' · 和值'+lotMetricText(lot,g.pick,'sum')+' · 跨度'+lotMetricText(lot,g.pick,'span')+'</div></div>');
  });
  h.push('</div></div>');
  h.push('<div class="irow">');
  ...
  pred5.forEach(function(pr){
    ...
    h.push('<tr><td style="color:#ffa500;font-weight:bold">'+pr.p+'</td><td>'+lotPickHtml([main],true)+'</td><td>'+lotPickHtml([extra],true)+'</td><td>'+lotMetricText(lot,g0,'sum')+'</td><td>'+lotMetricText(lot,g0,'span')+'</td><td>'+lotMetricText(lot,g0,'odd')+'</td><td>'+lotMetricText(lot,g0,'zone')+'</td><td><button class="expand-btn" onclick="togglePredDetail(\''+id+'\',\''+pr.p+'\')">展开</button></td></tr>');
    h.push('<tr class="pred-detail" id="'+id+'-detail-'+pr.p+'"><td colspan="8"><div class="pred-detail-box"><div style="text-align:right;margin-bottom:6px"><button class="copy-btn" onclick="copyLotteryPred(\''+id+'\',\''+pr.p+'\',true)">复制本期五组</button></div>');
  ...
  hist.slice(0,10).forEach(function(r){var main=r.pick[0]||{nums:[]},extra=r.pick[1]||{nums:[]};h.push('<tr><td>'+r.p+'</td><td>'+(r.d||'')+'</td><td>'+lotPickHtml([main],true)+'</td><td>'+lotPickHtml([extra],true)+'</td><td>'+drawSalesText(r)+'</td><td>'+lotMetricText(lot,r.pick,'sum')+'</td><td>'+lotMetricText(lot,r.pick,'odd')+'</td><td><button class="expand-btn" onclick="togglePredDetail(\''+id+'\',\'draw-'+r.p+'\')">查看详情</button></td></tr>');h.push('<tr class="pred-detail" id="'+id+'-detail-draw-'+r.p+'"><td colspan="8">'+prizeRowsHtml(id,r)+'</td></tr>')});
  h.push('</tbody></table></div>');
  h.push('<div class="rand-card"><div class="rand-head"><div class="rand-title">一键随机选号</div><div class="rand-ctrl"><span style="font-size:12px;color:#8899aa">生成组数</span><input type="number" id="'+id+'-lot-count" min="1" max="99" value="5"><button class="btn btn-o" onclick="genLotteryMany(\''+id+'\')">随机生成</button><button class="copy-btn" onclick="copyLotteryMany(\''+id+'\')">复制</button><button class="copy-btn" onclick="clearLotteryMany(\''+id+'\')">清空</button></div></div><div class="rand-out" id="'+id+'-lot-rand"><div style="text-align:center;color:#667788;font-size:12px;padding:8px">设置组数后点击随机生成</div></div></div>');
  h.push('<div class="icard"><h3>手动录入最新开奖</h3><div class="ig" id="'+id+'-manual"><input type="text" id="'+id+'-paste" placeholder="粘贴整组号码，如 4 1 7 7 2 或 6684890" style="min-width:220px"><button class="btn btn-o" onclick="fillLotteryPaste(\''+id+'\')">填入</button>'+manualHtml(lot,id)+'<button class="btn btn-o" onclick="submitLotteryR(\''+id+'\')">确认</button></div></div>');
  h.push('<div class="st" style="cursor:pointer" onclick="toggleLotteryH(\''+id+'\')">完整历史数据 ▾</div><div id="'+id+'-hist" style="display:none"></div>');
  h.push('<div class="rule-card open" id="rule-'+id+'"><div class="rule-head" onclick="toggleRule(\''+id+'\')"><span>'+lot.name+'玩法说明</span><span>展开/收起</span></div><div class="rule-body">'+lot.rule+lotPrizeTableHtml(id)+'</div></div>');
...
function toggleLotteryH(id){
  ...
  if(el.style.display==='none'||el.style.display===''){var h=['<div class="tw"><table><thead><tr><th>期号</th><th>日期</th><th>开奖结果</th><th>销量</th><th>详情</th></tr></thead><tbody>'];hist.forEach(function(r){h.push('<tr><td>'+r.p+'</td><td>'+(r.d||'')+'</td><td>'+lotPickHtml(r.pick,true)+'</td><td>'+drawSalesText(r)+'</td><td><button class="expand-btn" onclick="togglePredDetail(\''+id+'\',\'full-'+r.p+'\')">查看详情</button></td></tr>');h.push('<tr class="pred-detail" id="'+id+'-detail-full-'+r.p+'"><td colspan="5">'+prizeRowsHtml(id,r)+'</td></tr>')});h.push('</tbody></table></div>');el.innerHTML=h.join('');el.style.display='block'}else el.style.display='none';
}
...
function fetchJson(url){
  return fetch(url,{cache:'no-store',headers:{'Accept':'application/json,text/plain,*/*'}}).then(function(r){return r.text()}).then(function(t){try{return JSON.parse(t)}catch(e){throw '非JSON响应'}});
}
...
function fetchEastmoneyDirect(id){
  ...
  return fetch(url,{cache:'no-store'}).then(function(r){return r.text()}).then(function(html){var d=parseEastmoneyHtml(id,html);if(!d)throw 'empty eastmoney';return d});
}
...
function fetchShSportsDltDirect(){
  return fetch('https://www.shsportslottery.com/dlt/kjgg.html',{cache:'no-store'}).then(function(r){return r.text()}).then(function(html){var d=parseShSportsDltHtml(html);if(!d)throw 'empty shsports dlt';return d});
}
...
function fetchVipcKl8Direct(){
  ...
  return fetch('https://www.vipc.cn/result/kl8/'+p,{cache:'no-store'}).then(function(r){return r.text()}).then(function(html){var d=parseVipcKl8Html(html,p);if(!d)throw 'empty vipc kl8';return d});
}
