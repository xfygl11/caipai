...
(function(){
  ...
  function save(params,data){
    ...
    try{
      var serialized = JSON.stringify({time:Date.now(),data:data});
      // Android 16: 检查localStorage剩余空间，避免QuotaExceededError
      try {
        var testSize = serialized.length;
        var remaining = 5 * 1024 * 1024 - testSize; // 5MB估算上限
        if (remaining < 0) {
          console.warn('nc-cache: 数据过大，跳过缓存');
          return;
        }
        localStorage.setItem(key(params), serialized);
      } catch (sizeErr) {
        console.warn('nc-cache: localStorage空间不足，跳过缓存');
      }
    }catch(e){}
  }
