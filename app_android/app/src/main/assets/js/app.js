...
function renderTvCats(){
  ...
  if(window.NCDB&&movieState.dbCats&&movieState.dbCats.length){
    catsToRender=['推荐'].concat(movieState.dbCats);
  }else{
  ...
  var html=catsToRender.map(function(c){
    ...
    return '<button class="'+isActive+'" onclick="tvSetCat(\''+c.replace(/'/g,"\\'")+'\')">'+c+'</button>';
  }).join('');
  scroll.innerHTML=html;
  if(dropdown){
    var dropHtml=catsToRender.map(function(c){
      var isActive=movieState.cat===c?'active':'';
      return '<button class="'+isActive+'" onclick="tvSetCat(\''+c.replace(/'/g,"\\'")+'\');toggleCatDropdown()">'+c+'</button>';
    }).join('');
...
function tvSetCat(c){
  ...
  if(c==='推荐'){
    if(MOVIE_DATA&&MOVIE_DATA.length){grid.innerHTML='';renderMovieHome();return}
    else{MOVIE_DATA=[];renderMovieHome();return}
  }
  if(window.NCDB){
    ...
    NCDB.getSourceByBase(base).then(function(src){
      if(src&&src.id){
        NCDB.getMovies(src.id,c,60).then(function(list){
          ...
          else{MOVIE_DATA=[];renderMovieHome();updateLoadMoreBtn();setMovieStatus(c+' 暂无本地数据，请到「我的」页面采集',false);showEmptyGuide();}
        }).catch(function(){MOVIE_DATA=[];renderMovieHome();showEmptyGuide()});
      }else{MOVIE_DATA=[];renderMovieHome();updateLoadMoreBtn();setMovieStatus('请先采集数据',false);showEmptyGuide()}
    }).catch(function(){MOVIE_DATA=[];renderMovieHome();showEmptyGuide()});
    return;
  }
  MOVIE_DATA=[];renderMovieHome();
  ...
  NCDB.getLiveChannels().then(function(channels){
    if(!channels.length){listEl.innerHTML='<div style="text-align:center;color:#667788;padding:40px">暂无直播源</div>'}
      ...
      listEl.innerHTML=channels.map(function(ch){
        return '<div class="live-src-item"><div class="live-src-info"><b>'+ch.group+'</b><span>'+ch.name+'</span><small>'+ch.url.substring(0,50)+'...</small></div><div class="live-src-actions"><button class="live-src-btn" onclick="testLiveChannel(\''+ch.url+'\')">测试</button><button class="live-src-btn live-src-del" onclick="deleteLiveChannel('+ch.id+')">删除</button></div></div>';
      }).join('');
    }
  });
}
